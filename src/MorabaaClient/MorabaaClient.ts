...
import getPostsClient from "./posts/postsClient";
...
export const setMorabaaClientOptions = (
  options: Partial<IMorabaaClientOptions> | null = null
) => {
  if (options.customRoot) {
    MorabaaClientOptions.customRoot = options.customRoot;
    Object.entries(ClientRoots).forEach(([key, value]) => {
      ClientRoots[key] = `${options.customRoot}/api${
        ClientRoots[key].split("/api")[1]
      }`;
    });
  }
